        ...
        for y in range (-4,12):
            for z in range (-4,12):
                if (x in range(0,10) and y in range (0,10) and z in range (0,10)):
                    assert grid1.isInsideDomain(ne.Index_3d(x,y,z)), "everything inside [0,9] x [0,9] x [0,9] should be inside bGrid's domain"
                else:
                    assert not grid1.isInsideDomain(ne.Index_3d(x,y,z)), "everything outisde [0,9] x [0,9] x [0,9] should not be inside bGrid's domain"
                    
# def test_bGrid_dField_read():
#     backend = ne.Backend(ne.Backend.Runtime.openmp, 1)
#     grid = ne.bGrid(backend, ne.Index_3d(10,10,10))
#     field = grid.new_field(10)
#     for x in range (-1,11):
